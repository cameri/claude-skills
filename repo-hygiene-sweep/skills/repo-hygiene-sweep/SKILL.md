...
<workflow>
1. Run the script from anywhere — it resolves the workspace root from `$CLAUDE_PROJECT_DIR` (set by Claude Code), falling back to `REPO_HYGIENE_WORKSPACE_ROOT`; if neither is set it errors out rather than guessing a location.
2. It checks, in order: the workspace repo itself, `docs/` (a workspace-specific special case the root CLAUDE.md documents under `### `docs/` is a standalone repo` — skipped cleanly if it isn't one), then every `projects/...` bullet under the `### Standalone repos in `projects/`` heading.
...
<success_criteria>
Script exits 0 only when every checked repo is clean and (for git repos) not ahead of its remote. Exit 1 means at least one repo needs attention — the printed `DIRTY` lines say which and why — or that the standalone-repo list couldn't be parsed from CLAUDE.md (missing file or zero matching bullets), so a clean result can't be trusted.
</success_criteria>
