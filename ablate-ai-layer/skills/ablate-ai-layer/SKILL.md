...
- **Nothing is moved aside.** Every run happens in a detached git worktree built
  ...
  user's working tree is never modified — the only writes to the target repo are
  a gitignored `.ablation/` results dir and one `.gitignore` line — so there is
...
```bash
python3 <skill>/scripts/map_layer.py [repo_root]
```
...
cost, and that the working tree will not be touched (beyond the gitignored
`.ablation/` results dir and one `.gitignore` line).

```bash
python3 <skill>/scripts/run_ablation.py <repo> --task-file <task.md> --dry-run
```
...
```bash
python3 <skill>/scripts/run_ablation.py <repo> --task-file <task.md> --runs 2
```
