...
<step_3_root_compose>
Edit `$CONTAINERS_ROOT/compose.yml` and remove the include line:
...
<step_4_gatus>
Edit `$CONTAINERS_ROOT/gatus/config/config.yaml` and remove the endpoint entry for this service.

Gatus hot-reloads — no restart needed.
</step_4_gatus>

<step_5_tsdproxy>
**Remove from TsDproxy config (if present).**

Check all files in `$CONTAINERS_ROOT/tsdproxy/config/`:
- `applications.yaml`
- `admin-tools.yaml`
- `infrastructure.yaml`
- `automation.yaml`
...
<step_6_cloudflare>
If the service had a public Cloudflare tunnel entry, update the tunnel ingress via the API to remove it (see add-service.md's `<step_6_cloudflare>` for the API pattern).
</step_6_cloudflare>
