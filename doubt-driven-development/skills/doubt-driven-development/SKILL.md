...
In Claude Code, any Agent-tool `subagent_type` other than `fork` starts with genuinely isolated context — that's what this step needs. `general-purpose` is the default fresh reviewer; reach for a more specific installed subagent type (e.g. `code-reviewer`, `security-auditor`) when one matches the artifact's domain. Never use `fork` for this step — a fork inherits your conversation and would hand your own reasoning back to you dressed up as independent review.
...
**Non-interactive contexts** (cron jobs, webhook handlers, scheduled/autonomous runs):
