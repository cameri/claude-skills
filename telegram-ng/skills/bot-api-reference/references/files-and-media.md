...
- **`file_id` is bot-scoped.** If `server.ts` ever rotates or swaps the bot token (e.g. moving from one instance's bot to a different one), any `file_id` values persisted from the old bot become unusable — they need to be re-fetched from the source message, not just re-sent.
