#!/usr/bin/env bash
# Sends /rename <name> + Enter to the tmux pane running this Claude Code session.
set -euo pipefail
...
if [ -z "${TMUX:-}" ]; then
  echo "Not running inside tmux — nothing to rename." >&2
  exit 1
fi
...
tmux send-keys -t "$pane_id" -l -- "/rename $1"
