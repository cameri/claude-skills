#!/usr/bin/env bash
# Sends /background + Enter to the pane running this Claude Code session
...
if [ -z "$(pane_io_active)" ]; then
  echo "Not running inside tmux or herdr — nothing to background." >&2
  exit 1
fi
...
pane_io_send "$pane_id" "/background"
