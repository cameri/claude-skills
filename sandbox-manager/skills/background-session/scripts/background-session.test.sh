#!/usr/bin/env bash
# Tests for background-session.sh, using stub tmux/herdr executables on PATH.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET="$SCRIPT_DIR/background-session.sh"
...
assert_eq "(c) sends /background literally" "-t${us}%3${us}-l${us}--${us}/background${us}" "$(sed -n '1p' "$TMUX_STUB_LOG")"
...
assert_eq "(d) agent prompt sends pane id and /background" "w1:p1${us}/background${us}" "$(sed -n '1p' "$HERDR_STUB_LOG")"
teardown
