#!/usr/bin/env bash
# Sends /branch + Enter to the tmux pane running this Claude Code session.
set -euo pipefail

if [ -z "${TMUX:-}" ]; then
  echo "Not running inside tmux — nothing to branch." >&2
  exit 1
fi
...
tmux send-keys -t "$pane_id" "/branch" Enter
