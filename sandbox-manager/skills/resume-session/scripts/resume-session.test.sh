#!/usr/bin/env bash
# Tests for resume-session.sh, using stub tmux/herdr executables on PATH.
set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET="$SCRIPT_DIR/resume-session.sh"
...
assert_eq "(c) sends /resume with the name literally" \
  "-t${us}%5${us}-l${us}--${us}/resume my-session${us}" "$(sed -n '1p' "$TMUX_STUB_LOG")"
...
assert_eq "(d) agent prompt sends pane id and /resume with the name" \
