...
assert_eq "(d) pane-run sends pane id and /exit" "w1:p1${us}/exit${us}" "$(sed -n '1p' "$HERDR_STUB_LOG")"
teardown
...
assert_eq "(e) sent /exit via pane run" "w1:p1${us}/exit${us}" "$(sed -n '1p' "$HERDR_STUB_LOG")"
