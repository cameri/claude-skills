/**
 * omp (Oh My Pi) wake bridge for the <CHANNEL> channel.
 *
 ...
 *
 * Wakes are content-bearing (the message text is the prompt). The wake uses
 * a bare `pi.sendUserMessage(wrapped)` call — NO options — because omp only
 ...
 * and never wakes an idle session.
 *
 * `pi.sendUserMessage` is SYNCHRONOUS: it returns undefined, never a
 * Promise. Do not chain `.catch` on it — that throws `undefined is not an
 * object (evaluating 'pi.sendUserMessage(wrapped).catch')` on every wake.
 * Use the try/catch below instead.
 *
 * The server MUST set `meta.source` on every channel notification; the
 * filter below accepts this plugin's own server name or that source. A
 * notification with neither is silently dropped.
 *
 * Extensions are loaded once at session start — after editing this file,
 * restart the session before testing.
 */
...
/** Channel source value the server declares in its notification meta. */
const SOURCE_NAME = "<channel-name>";
...
export default function ompChannelBridge(pi: ExtensionAPI): void {
  pi.on("mcp_notification", (event) => {
    ...
    // Accept the server name (renamed .mcp.json) or the source this plugin
    // declares in its own meta.
