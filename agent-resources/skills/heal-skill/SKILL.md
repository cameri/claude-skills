---
name: heal-skill
description: Apply corrections to a skill's SKILL.md and related files based on mistakes discovered during execution, with user approval before any edit. Use when a skill's instructions turned out to be wrong, outdated, or led to an error, and the fix should be written back into the skill itself.
user-invocable: true
allowed-tools:
  - Read
  - Edit
